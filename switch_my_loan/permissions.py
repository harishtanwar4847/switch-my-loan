...
def lead_query(user = frappe.session.user):
    pass
    if (('CRM User' in frappe.get_roles(user) or 'Partner User' in frappe.get_roles(user)) and not 'Sales Manager' in frappe.get_roles(user) and not 'System Manager' in frappe.get_roles(user)):
